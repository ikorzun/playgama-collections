...
| Card facts | identical on both: rating, then `platforms / date / developer`, then the series — no labels |  |
...
Card facts follow `9108:45841`: a rating row, then `platforms / date / developer` with the separators
dimmed, then the series — no field labels on either width. The stars fill proportionally to
`ratingValue`, and the votes beside them are `ratingCount`, the same number the likes pill abbreviates.
Playgama has no developer pages — the game page prints the name as plain text — so the credit links to a
Playgama search for that developer, which does return their games.
